...
export default function TabelaItensPrescricao({ itens, onReorder, onRemove, readOnly = false }: Props) {
    if (itens.length === 0) {
        return (
            <Paper sx={{ p: 3, mb: 1.5, textAlign: 'center', bgcolor: '#fafafa', border: '1px dashed #e0e0e0', borderRadius: 2, boxShadow: 'none' }}>
                <Typography variant="body2" color="text.secondary">
                    Nenhum cuidado prescrito.
                </Typography>
            </Paper>
        );
    }
            ...
            <Table size="small">
                <TableHead>
                    <TableRow sx={{ bgcolor: '#f8f9fa' }}>
                        ...
                        <TableCell sx={{ color: 'text.primary', fontWeight: 600, fontSize: '0.75rem', py: 1.5 }}>Cuidado / Item</TableCell>
                        ...
                        <React.Fragment key={item.idItem}>
                            ...
                            {item.observacoes && (
                                <TableRow sx={{
                                    ...
                                    <TableCell colSpan={6} sx={{ py: 0.5, pb: 1.5, pt: 0 }}>
